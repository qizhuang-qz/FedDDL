...
class DINOSAM():
    ...
    def __call__(self, image, label, content, f_name, groundingdino_model, w):
        ...
        Bx_temp = []
        # ipdb.set_trace()
        f_name_1 = f_name  #  ../Datasets/NICO/0dog/0on grass/1.jpg
        ...
        category_path_1 = '{}/{}'.format(category_path, folder_name)  # 0/0 on grass
...
#         ipdb.set_trace()
        ...
        image,phrases = extract_bbox_region(image_source, boxes, logits, phrases)
        ...
        for i, (image, phrase) in enumerate(zip(image, phrases), start=1):
            try:
                cv2.imwrite(f'{rfb_path}/{i}_{phrase}.jpg', image[..., ::-1])
            except:
                print('empty image!')
...
def extract_bbox_region(image, bbox_ls, logits, phrases, max_len=2):
...
#         bbox_region = image[y1:y2, x1:x2] # y1=197 y2=367  x1=52 x2=190
        ...
        if selected_region is None or selected_region.size == 0:
            print("Selected region is None or empty")
            continue 

# 创建随机噪声
        noise = np.random.randint(0, 256, selected_region.shape, dtype=np.uint8)

# 将噪声添加到选定区域
        noisy_region = cv2.add(selected_region, noise)
...
# 将带噪声的区域放回原图像
    ...
    """
        if bbox_region.shape[0] == 0 or bbox_region.shape[1] == 0:
            ...
            """
    return image_regions,valid_phrases
...
if __name__ == '__main__':
    ...
    stage = "train"
    dinosam = DINOSAM("NICO_Contextual_1")  #   f"./{stage.upper()}_REGIONS_BY_PREDICTING_TOP6_ANOTHER"
...
#     ipdb.set_trace()
    ...
    # height, width = image1.shape[:2]
    # target_size = (width, height)
    with open(f'regions_{stage}_by_predicting_top6_another.txt', 'w', encoding='utf-8') as w:
        
        base_dir =  '../Datasets/NICO/9monkey'
